...
main() {
    ...
    export FABRIC_VERSION=${FABRIC_VERSION:-$(getVarFromDotEnvFile 'FABRIC_VERSION' "${FABRIC_DIR}/.env")}
    export FABRIC_STARTER_VERSION=${FABRIC_STARTER_VERSION:-$(getVarFromDotEnvFile 'FABRIC_STARTER_VERSION' "${FABRIC_DIR}/.env")}
    export FABRIC_STARTER_REPOSITORY=${FABRIC_STARTER_REPOSITORY:-$(getVarFromDotEnvFile 'FABRIC_STARTER_REPOSITORY' "${FABRIC_DIR}/.env")}
...
function getVarFromDotEnvFile() {
    local varName=${1}
    local filePath="${2}"
    
    bash -c "export $(cat ${filePath} | grep -v '^#'| cut -d= -f1,2) ; echo \${${varName}}"
}
